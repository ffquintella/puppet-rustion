...
| `bastionvault_bind` | `String` | `'0.0.0.0:9443'` | TLS listen address for the control plane |
| `bastionvault_tls_cert` | `Optional[Stdlib::Absolutepath]` | `undef` | TLS certificate path (required when enabled) |
| `bastionvault_tls_key` | `Optional[Stdlib::Absolutepath]` | `undef` | TLS private key path (required when enabled) |
| `bastionvault_identity_pub` | `Optional[Stdlib::Absolutepath]` | `${config_dir}/control-plane/identity.pub` | Hybrid Ed25519 + ML-DSA-65 public key BastionVault pins |
| `bastionvault_identity_priv` | `Optional[Stdlib::Absolutepath]` | `${config_dir}/control-plane/identity.key` | Hybrid private key (generated by `rustion control-plane identity rotate`) |
...
| `bastionvault_recording_fetch_enabled` | `Boolean` | `true` | Allow BastionVault to fetch recording bytes |
| `bastionvault_active_session_cap` | `Integer` | `512` | Concurrent control-plane session cap |
| `bastionvault_max_envelope_bytes` | `Integer` | `16384` | Maximum BVRG-v1 envelope size accepted |
| `bastionvault_health_rate_per_source_per_sec` | `Integer` | `4` | Per-source IP `/v1/health` rate limit |
| `bastionvault_health_rate_per_authority_per_sec` | `Integer` | `10` | Per-authority `/v1/health` rate limit |
